...
const Sidebar: React.FC<{ activeView: AdminView; setActiveView: (view: AdminView) => void; user: User }> = ({ activeView, setActiveView, user }) => {
  const navItems = [
    { id: 'overview', label: 'Overview', icon: Icons.LayoutDashboardIcon, feature: 'admin_dashboard' },
    { id: 'staff', label: 'Staff Management', icon: Icons.UsersIcon, feature: 'admin_dashboard' },
    { id: 'facility', label: 'Facility Management', icon: Icons.BuildingIcon, feature: 'admin_dashboard' },
    { id: 'organizations', label: 'Org Hierarchy', icon: Icons.BuildingIcon, feature: 'multi_tenancy' },
    { id: 'subscription', label: 'Subscription', icon: Icons.DollarSignIcon, feature: 'admin_dashboard' },
    { id: 'data', label: 'Data Management', icon: Icons.DatabaseIcon, feature: 'data_io' },
    { id: 'audit', label: 'Audit Log', icon: Icons.ClipboardListIcon, feature: 'audit_log' },
  ];

  const NavLink: React.FC<{ item: typeof navItems[0] }> = ({ item }) => {
    if (!canAccessFeature(user, item.feature)) return null;
    return <button onClick={() => setActiveView(item.id as AdminView)} className={`sidebar-link ${activeView === item.id ? 'active' : ''}`}><item.icon /><span>{item.label}</span></button>;
  };
...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  const renderContent = () => {
    if (isLoading || !data) return <FullScreenLoader message="Loading administrator dashboard..." />;
    
    switch (activeView) {
      case 'overview': return <AdminDashboardOverview staffCount={data.staff.length} patientCount={data.patients.length} appointmentCount={data.appointments.length} totalRevenue={data.totalRevenue} />;
      case 'staff': return <StaffManagementView staff={data.staff} organizations={data.organizations} departments={data.departments} onUpdateUser={handleUpdateUser} currentUser={props.user} />;
      case 'organizations': return <OrganizationManagementView organizations={data.organizations} onUpdate={fetchData} currentUser={props.user} />;
